export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      allowNetworkHint: 'When on, scripts may access the network: the Docker container is attached to the network and network libraries (requests/urllib/http.client) pass validation. When off, scripts using those libraries are blocked before execution.',
      localNetworkDisabledWarning: 'The local process sandbox cannot reliably block network at the process level, so scripts will be refused when this is off. Use the Docker backend if you need network isolation.',
