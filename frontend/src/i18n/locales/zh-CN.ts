export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      allowNetwork: '允许网络访问',
      allowNetworkHint: '开启后脚本可访问网络：Docker 容器接入网络并放行 requests/urllib/http.client 等网络库；关闭时使用这些库的脚本会在执行前被安全校验拦截。',
      localNetworkDisabledWarning: '本地进程沙箱无法在进程级阻断网络，关闭后脚本将被拒绝执行。如需网络隔离请改用 Docker 后端。',
