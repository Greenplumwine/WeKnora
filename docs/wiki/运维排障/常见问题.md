...
- **安全模式（推荐）**：生产环境不要把 `em_…` 写在页面 HTML 里；由业务后端提供 `token-endpoint`，用发布 Token 调 `POST /api/v1/embed/:id/exchange` 换取短时令牌 `ems_…`（约 30 分钟有效）。详见 [网页嵌入集成](../集成扩展/网页嵌入集成.md)。
- **限流**：渠道可配置每分钟 / 每日请求上限；超限返回 429。
- **子域部署**：若 embed 页面与 API 不同子域，参考 [网页嵌入集成](../集成扩展/网页嵌入集成.md) 配置 CORS 与 Nginx。

> Embed 组件基础接入参见 [网页嵌入集成](../集成扩展/网页嵌入集成.md)
