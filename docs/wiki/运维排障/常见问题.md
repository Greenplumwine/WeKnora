...
通常是 Embedding 模型和对话模型没有正确被设置导致。按照以下步骤排查：

1. 查看 `.env` 配置中的模型信息是否配置完整，其中如果使用 ollama 访问本地模型，需要确保本地 ollama 服务正常运行，同时在 `.env` 中的如下环境变量需要正确设置：
...
如果是通过 remote api 访问模型，则需要额外提供对应的 `BASE_URL` 和 `API_KEY`：
...
> 搜索引擎的 API 端点硬编码策略参见 [添加网络搜索引擎](../集成扩展/添加网络搜索引擎.md)
...
> 完整的调优参数说明参见 [Langfuse集成](../集成扩展/Langfuse集成.md)
...
如需临时回退到「仅审计、不拦截」灰度窗口，可在配置里设置 `tenant.enable_rbac=false`（或环境变量 `WEKNORA_TENANT_ENABLE_RBAC=false`）。完整的角色矩阵和归属链请见 [空间RBAC说明](../安全认证/RBAC说明.md)。
...
按照 [空间RBAC说明](../安全认证/RBAC说明.md) 的角色矩阵：
...
- 集成测试与索引映射细节见 [OpenSearch集成测试](../开发部署/OpenSearch集成测试.md)。
...
- 详见 [knowledge API](../API参考/knowledge.md)。
...
- **安全模式（推荐）**：生产环境不要把 `em_…` 写在页面 HTML 里；由业务后端提供 `token-endpoint`，用发布 Token 调 `POST /api/v1/embed/:id/exchange` 换取短时令牌 `ems_…`（约 30 分钟有效）。详见 [Embed安全模式](../集成扩展/Embed安全模式.md) 与 [Embed子域部署](../集成扩展/Embed子域部署.md)。
- **限流**：渠道可配置每分钟 / 每日请求上限；超限返回 429。
- **子域部署**：若 embed 页面与 API 不同子域，参考 [Embed子域部署](../集成扩展/Embed子域部署.md) 配置 CORS 与 Nginx。

> Embed 组件基础接入参见 [Embed组件集成](../集成扩展/Embed组件集成.md)
...
- 若创建知识库时提示存储引擎不可用，请确认目标 provider 在 `STORAGE_ALLOW_LIST` 允许范围内。详见 [storage-backend API](../API参考/storage-backend.md)。
...
- 详见 [Worker池治理](../开发部署/Worker池治理.md)。注意：Worker 并发只是调度预算，仍受模型配额、DocReader 容量、向量库与数据库连接数限制。
...
> IM 集成开发参见 [IM集成开发](../集成扩展/IM集成开发.md)
...
如果以上方式未解决问题，请在 issue 中描述您的问题，并提供必要的日志信息辅助我们进行问题排查。

## 相关主题

- [开发指南](../开发部署/开发指南.md) - 开发环境问题排查
- [知识图谱](../核心功能/知识图谱.md) - 知识图谱功能故障排查
- [内置模型管理](../核心功能/内置模型管理.md) - 模型配置问题
- [IM集成开发](../集成扩展/IM集成开发.md) - IM 集成相关问题
- [空间RBAC说明](../安全认证/RBAC说明.md) - 权限相关问题
- [日志配置](日志配置.md) - 日志详细配置
...
- [知识图谱](../核心功能/知识图谱.md) - 知识图谱功能相关故障排查
