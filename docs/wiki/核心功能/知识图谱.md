...
aliases: [KnowledgeGraph, 图谱, 知识图谱配置, KnowledgeGraphSetup, 开启知识图谱]
---
...
- `NEO4J_ENABLE` 是知识图谱功能的唯一全局开关，设置为 `true` 才会启用知识图谱相关逻辑
...
在知识库或对话页面中上传文档后，前端应展示图谱可视化入口。对话时系统会自动查询知识图谱，并获取相关知识返回补充信息。
...
- [内置模型管理](内置模型管理.md) - 实体/关系抽取使用的模型配置（抽取用 SummaryModelID）
