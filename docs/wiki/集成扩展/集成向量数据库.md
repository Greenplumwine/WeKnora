...
> 类似的扩展开发模式参见 [添加网络搜索引擎](添加网络搜索引擎.md)
...
RETRIEVE_DRIVER=postgres,elasticsearch,your_database
...
建议参考现有的 PostgreSQL 和 Elasticsearch 实现作为开发模板。这些实现位于 `internal/application/repository/retriever/` 目录下：

- PostgreSQL: `postgres/`
- Elasticsearch V7 / V8: `elasticsearch/v7/`、`elasticsearch/v8/`
- Apache Doris 4.1: `doris/`
...
当前支持的检索引擎类型常量定义在 `internal/types/retriever.go`，包括 `postgres`、`elasticsearch`、`infinity`、`elasticfaiss`、`qdrant`、`milvus`、`weaviate`、`doris`、`sqlite`、`tencent_vectordb`、`opensearch`。
...
- [添加网络搜索引擎](添加网络搜索引擎.md) - 同类扩展开发模式（接口实现 + 注册 + DI）
