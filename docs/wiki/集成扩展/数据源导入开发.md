...
WeKnora 的数据源导入模块支持从外部平台（飞书/Lark、Notion、语雀、RSS 等）自动导入和同步内容到知识库。用户可配置数据源连接，选择需要同步的资源，并通过手动触发或定时调度自动完成内容的增量/全量同步。

数据源绑定到知识库，一个知识库可接入多个数据源。所有配置通过前端知识库设置页面管理，凭证使用 AES-256-GCM 加密存储在数据库中。

> 数据源导入与 [IM集成开发](IM集成开发.md) 都涉及飞书集成，可考虑共享飞书应用凭证

## 当前支持的连接器

| 连接器 | 类型标识 | 认证方式 | 增量同步 | 删除同步 |
|--------|---------|---------|:-:|:-:|
| 飞书 (Feishu) | `feishu` | OAuth2 (Tenant Access Token) | ✅ | ✅ |
| Lark | `lark` | OAuth2 (Tenant Access Token) | ✅ | ✅ |
| Notion | `notion` | API Key (Integration Token) | ✅ | - |
| 语雀 (Yuque) | `yuque` | API Key (Token) | ✅ | - |
| RSS / Atom | `rss` | 自定义（可选请求头） | ✅ | - |

> 删除同步指连接器能检测源端删除并计入同步统计；出于数据安全考虑，WeKnora **不会**据此自动删除本地知识，需用户在知识库界面手动管理（详见 [同步处理流程](#同步处理流程)）。
>
> Lark（飞书国际版）与飞书共用同一套连接器代码，能力相同，详见下文 [Lark 知识库接入](#lark-知识库接入)。
>
> 此外 `ConnectorMetadataRegistry` 中还登记了 Confluence、GitHub、Google Drive、OneDrive、DingTalk、Slack、IMAP、Web Crawler 等类型（`GET /datasource/types` 会返回），但尚未实现连接器，创建对应类型的数据源会返回 `ErrConnectorNotFound`。本文档的接入指南仅覆盖飞书/Lark。
...
在 **权限管理** 中搜索并开通以下权限（均为只读，符合最小权限原则）：

| 权限点 | 用途 | 对应 API |
|--------|------|----------|
| `wiki:wiki:readonly` | 读取知识库空间列表与节点树 | `wiki/v2/spaces`、`wiki/v2/spaces/{id}/nodes`、`wiki/v2/spaces/get_node` |
| `docx:document:readonly` | 读取新版文档内容 | `docx/v1/documents/{id}/raw_content` |
| `drive:drive:readonly` | 下载云盘上传的文件（PDF/Word/图片等） | `drive/v1/files/{token}/download` |
| `drive:export:readonly` | 异步导出文档/表格/多维表格为 docx/xlsx | `drive/v1/export_tasks`（创建/轮询/下载） |

> **注意**：`drive:drive:readonly` 与 `drive:export:readonly` 是两个独立权限，互不覆盖。同步知识库时两类操作都会发生：docx/sheet/bitable 走异步导出（需 `drive:export:readonly`），obj_type=file 的节点走直接下载（需 `drive:drive:readonly`），缺一会导致部分文档同步失败。

**知识库授权**（权限点之外的关键步骤）：光开通权限点不够，飞书还有文件可见范围机制。需在目标知识空间「设置 -> 成员管理」把应用加为成员（「可阅读」及以上），或在应用后台「应用功能 -> 知识库」配置可见范围，确保目标空间对应用可见，否则读取节点/下载文件会返回权限错误。
...
权限标识与飞书相同，在 Lark 开放平台的 **权限管理** 中开通（均为只读）：

| 权限点 | 用途 | Lark 官方文档核对 |
|------|------|------|
| `wiki:wiki:readonly` | 读取知识库空间列表与节点树 | ✅ 已核对 |
| `docx:document:readonly` | 读取新版文档内容 | ⚠️ 未能核对（Lark 文档页渲染受限），沿用飞书侧标识 |
| `drive:drive:readonly` | 下载云盘上传的文件（PDF/Word/图片等） | ✅ 已核对 |
| `drive:export:readonly` | 异步导出文档/表格/多维表格为 docx/xlsx | ✅ 已核对 |

> `drive:drive:readonly` 与 `drive:export:readonly` 是两个独立权限，互不覆盖，同步知识库时缺一会导致部分文档失败。详见飞书侧的[权限说明](#第二步开通权限)。
>
> **不要套用 [IM 集成文档](IM集成开发.md#飞书接入)里的那份权限 JSON**：那份清单混入了
> `aily:file:*`、`corehr:file:download` 等在 Lark 并不存在的权限（整份导入会失败），
> 且反而**缺少**上表中数据源真正需要的 `drive:*` / `docx:*`。
>
> 若同一个 Lark 应用既做 IM 机器人又做知识库同步，请使用
> [IM 集成文档 - Lark 权限配置](IM集成开发.md#lark-接入)中的合并清单。
...
CREATE TABLE data_sources (
    ...
    status                  VARCHAR(32) DEFAULT 'active',  -- 'active' | 'paused' | 'error' | 'deleted'
    conflict_strategy       VARCHAR(32) DEFAULT 'overwrite',    -- 'overwrite' | 'skip'
...
| 连接器类型 | 凭证字段（Credentials） | 认证方式 |
|-----------|---------|---------|
| 飞书 (feishu) / Lark (lark) | `app_id`, `app_secret` | OAuth2 (Tenant Access Token) |
| Notion (notion) | `api_key`（Internal Integration Token） | API Key |
| 语雀 (yuque) | `api_token`（个人 Token） | API Key |
| RSS (rss) | `auth_headers`（可选，自定义请求头） | 自定义（feed_urls 存放在 Settings，非密钥） |
...
CREATE TABLE sync_logs (
    ...
    tenant_id       BIGINT NOT NULL,
    status          VARCHAR(32) NOT NULL,    -- 'running' | 'success' | 'partial' | 'failed' | 'canceled'
...
| GET | `/api/v1/datasource/:id/resources?parent_id=xxx` | 浏览外部系统的可用资源（支持懒加载子节点） |
| POST | `/api/v1/datasource/:id/resource-ancestors` | 解析资源祖先 ID（回显懒加载选择器中的已选资源） |
...
凭证独立于数据源主体配置，通过专属子资源管理（更新/删除数据源主体时不会改动凭证）：

| 方法 | 路径 | 说明 |
|------|------|------|
| PUT | `/api/v1/datasource/:id/credentials` | 整体替换凭证 map（原子操作） |
| DELETE | `/api/v1/datasource/:id/credentials/:field` | 删除凭证中的单个字段 |
...
- **同步删除 (SyncDeletions)**：源端删除文档时是否将删除计入同步统计。出于数据安全考虑，WeKnora 不会据此自动删除本地知识，需用户在知识库界面手动清理
...
| `ListResources` | 列出外部系统中可选的资源（如知识库空间、文件夹），支持 `parentID` 懒加载子节点 |
| `ResolveResourceAncestors` | 解析资源祖先 ID，用于懒加载选择器回显深层嵌套的已选资源 |
...
type Resource struct {
    ...
    HasChildren bool                   // 是否有子节点（驱动前端懒加载）
...
│  · 手动同步 -> 队列: sync（无 TaskID，不去重）        │
│  · 定时同步 -> 队列: sync, TaskID 去重               │
└──────────────────────────┬────────────────────────┘
                           │
                           ▼
...
FetchedItem
    │
    ├─ IsDeleted = true?
    │     └─ SyncDeletions=true 时计入 items_deleted（仅计数，
    │        不实际删除本地知识，需用户在 KB 界面手动清理）
    │
    ├─ 有文件内容 (Content)?
...
| `ListResources()` | 列出外部系统中可选的资源。`parentID` 为空返回顶层资源，非空时只返回该资源的直接子节点（懒加载） |
| `ResolveResourceAncestors()` | 对给定资源 ID 解析其所有祖先 ExternalID，用于懒加载选择器回显已选中的深层嵌套资源 |
...
type DataSourceService interface {
    ...
    UpdateDataSourceCredentials(ctx context.Context, id string, credentials map[string]interface{}) (*types.DataSource, error)
    ...
    ListAvailableResources(ctx context.Context, dataSourceID string, parentID string) ([]types.Resource, error)
    ResolveResourceAncestors(ctx context.Context, dataSourceID string, resourceIDs []string) ([]string, error)
...
| `AuthType` | 认证方式：`oauth2` / `api_key` / `token` / `password` / `custom` / `none` |
...
```
DownloadDriveFile -> GET /open-apis/drive/v1/files/{token}/download
```
...
| `internal/datasource/connector/feishu/region.go` | 区域定义（Feishu / Lark 两朵云的 API 与 Web 地址） |
...
| 参数 | 值 | 说明 |
|------|------|------|
| Token 缓存安全余量 | 5 min | Token 过期前提前刷新（飞书） |
| 飞书 Wiki Space 分页大小 | 50 | ListResources 每页返回数 |
| 飞书 Wiki 节点分页大小 | 50 | ListWikiNodes 每页返回数 |
...
| 定时同步队列 | `sync` | asynq 队列（types.QueueSync），含 TaskID 去重 |
| 手动同步队列 | `sync` | asynq 队列（types.QueueSync），无 TaskID |
| 同步任务超时 | 2h | asynq Timeout |
| 同步任务最大重试 | 5 | asynq MaxRetry |
...
| 数据源非活跃状态触发同步 | 仅允许 `active`、`error` 或 `paused` 状态手动同步 |
...
func (c *Connector) ListResources(ctx context.Context, config *types.DataSourceConfig, parentID string) ([]types.Resource, error) {
    // 列出可选资源（工作区、空间、文件夹等）
    // parentID 为空返回顶层，非空时返回该资源的直接子节点（懒加载）
}

func (c *Connector) ResolveResourceAncestors(
    ctx context.Context, config *types.DataSourceConfig, resourceIDs []string,
) ([]string, error) {
    // 返回需要展开以回显已选资源的祖先 ExternalID
    // 已返回完整树或扁平列表的连接器可直接返回 nil
    return nil, nil
}
...
func initConnectorRegistry() (*datasource.ConnectorRegistry, error) {
    ...
    if err := registry.Register(myplatform.NewConnector()); err != nil { // 新增
        errs = errors.Join(errs, fmt.Errorf("register myplatform connector: %w", err))
    }
    if errs != nil {
        return nil, errs
    }
...
- [IM集成开发](IM集成开发.md) - 同样涉及飞书集成，可共享应用凭证
- [共享空间说明](../安全认证/共享空间说明.md) - 知识库共享与数据源导入互补
- [添加网络搜索引擎](添加网络搜索引擎.md) - 同类扩展开发模式
