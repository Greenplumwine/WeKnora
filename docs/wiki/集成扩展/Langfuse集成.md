---
title: Langfuse集成
tags: [集成扩展, Langfuse, 可观测性, 追踪, 监控]
aliases: [Langfuse, 可观测性, Tracing]
---
...
- **跨进程 trace 透传**：HTTP 层把 W3C `traceparent` 注入 asynq payload，worker 在 asynq middleware 层自动 extract 并 resume 同一条 trace；定时任务（例如数据源同步）则退化为独立 trace，依然按任务类型（`asynq.<type>`）聚合。
...
| --- | --- | --- |
| Trace | 一次 HTTP 请求（含其触发的所有 asynq 任务） | 对于 `knowledge-chat`、`agent-chat`、`knowledge-search`、`generate_title`、`evaluation`、模型连通性测试等在线请求；以及文件上传/URL 入库/manual/reparse/move/copy、FAQ 导入、知识修改、wiki auto-fix、数据源手工触发等入库请求，HTTP 层都会开启 trace，并把 W3C `traceparent` 注入 asynq payload。 |
| Span（type=SPAN） | 每个 asynq 任务的执行窗口 / 每次 Agent 执行及其每一轮 / 每次工具调用 | 由 `internal/tracing/langfuse.AsynqMiddleware` 在 `mux.Use` 注册；对每个 handler 自动创建 `asynq.<task_type>` 的 SPAN，并记录 `task_id` / `queue` / `retry` / `max_retry` / `payload_bytes`。定时任务（无上游 trace）会退化为 `asynq.<task_type>` 独立 trace。**Agent 相关**：`AgentEngine.Execute` 会开 `agent.execute` 顶层 SPAN，其下每一轮 ReAct 循环开 `agent.round.N` SPAN，每次工具调用开 `agent.tool.<tool_name>` SPAN（参数、输出、耗时、成败、错误都会写入）。 |
| Generation（type=GENERATION） | 每次 chat / embedding / rerank / VLM / ASR 调用 | 若位于 span 下会自动作为该 span 的子节点（通过 OTel span 上下文自动父子关联），所以 Langfuse UI 呈现 trace -> asynq-span -> generation 的树状结构；Agent 模式下是 trace -> agent.execute -> agent.round.N -> (chat.completion.stream + agent.tool.X -> rerank/embedding...) 的完整树。 |
...
| `userId` | 认证用户 ID（由 auth 中间件写入 ctx） / 空间 ID | 未登录时退化为 `tenant:<id>`，方便按空间汇总消耗；enqueue 时会写入 payload，worker 在无上游 trace 的场景也能保留归属。 |
...
下表列出当前会在 Langfuse 里自动出现对应 SPAN 的 asynq 任务；每种任务的 payload 均已嵌入 `types.TracingContext`，enqueue 时由 `langfuse.InjectTracing(ctx, &payload)` 从当前 HTTP trace 拷出 W3C `traceparent`（写入 payload 的 `lf_traceparent` 字段）。

| 任务类型常量 | Handler | 典型触发来源 |
| --- | --- | --- |
| `document:process` | `knowledgeService.ProcessDocument` | 文件 / URL / 文本 / file_url 四种入库；reparse；知识库克隆内部重派发 |
| `temporary_document:process` | `TemporaryDocument.Process` | 会话临时文档解析（聊天中 @ 临时文件） |
| `manual:process` | `knowledgeService.ProcessManualUpdate` | 手工知识新建 / 更新 |
...
| `knowledge:move` / `knowledge:list_delete` / `knowledge:list_reparse` / `index:delete` / `kb:clone` / `kb:delete` | 知识移动 / 批量删除 / 批量重解析 / 索引清理 / 知识库复制 / 知识库删除 | 对应 HTTP 路由 |
| `wiki:ingest` | `wikiIngestService.ProcessWikiIngest` | Wiki auto-fix / 重建链接 |
| `wiki:finalize` | `wikiIngestService.Handle`（复用 wiki:ingest handler） | Wiki KB 级收尾（防抖触发：索引重建 / 死链清理 / 交叉链接注入） |
| `datasource:sync` | `DataSourceService.ProcessSync` | 数据源手动触发 + 定时调度（定时场景下 trace 为 standalone） |
...
| Rerank | `rerank` | 按 `query + 所有文档` 的 rune 数估算 input tokens | 输出只上报前 50 条 `(rank, index, model_score)`。 |
...
- `internal/tracing/langfuse/` - Langfuse 客户端、异步批量上报、Gin 中间件、**asynq middleware**、Span / Trace resume 实现。
  ...
  - `asynq.go` - `AsynqMiddleware()` 统一在 mux 上包 handler；`InjectTracing(ctx, payload)` 在 enqueue 侧把 W3C `traceparent` 注入 payload。
  - `middleware.go` - Gin 中间件 + `shouldTrace` 白名单（覆盖 chat / 入库 / FAQ / wiki / 数据源等路径）。
- `internal/types/tracing.go` - `TracingContext` POCO，所有 asynq payload 通过嵌入此结构携带 `lf_traceparent` / `lf_trace_id` / `lf_parent_obs_id` / `lf_user_id` / `lf_session_id`（其中 `lf_traceparent` 是 OTLP 路径实际使用的 W3C traceparent，`lf_trace_id` / `lf_parent_obs_id` 仅作向后兼容保留）。
...
- [常见问题](../运维排障/常见问题.md) - FAQ 中有 Langfuse 开启步骤
- [Worker池治理](../开发部署/Worker池治理.md) - asynq worker 池与 trace 透传的治理配合
