---
title: Embed子域部署
tags: [集成扩展, Embed, 部署, 子域, Nginx]
aliases: [embed-subdomain, 子域部署, 独立子域]
---
...
2. **你的业务后端源站**（安全模式时建议填）--你自己写的「取令牌接口」在服务端调用 `POST .../exchange` 时，需带与白名单一致的 `Origin` 头（见 [Embed安全模式](Embed安全模式.md)）
...
- [Embed组件集成](Embed组件集成.md) - Embed 组件的基础接入流程
- [Embed安全模式](Embed安全模式.md) - 发布 Token 不落浏览器的安全模式
...
- [Embed组件集成](Embed组件集成.md) - 基础接入
- [Embed安全模式](Embed安全模式.md) - 安全模式与域名白名单
