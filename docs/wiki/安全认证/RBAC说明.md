...
除了空间内的四种角色，还有三种特殊身份绕过或补充常规角色矩阵：

#### 跨空间超管（Cross-tenant Superuser）

满足**两个条件同时成立**才生效：

- 用户属性 `User.CanAccessAllTenants = true`（数据库用户字段，由平台运营方授予）
- 集群开关 `tenant.enable_cross_tenant_access = true`（默认 `false`）

两者缺一不可--`enable_cross_tenant_access=false` 时，即便 `CanAccessAllTenants=true` 的用户也只能在自己 home 空间操作。这是因为 `IsCrossTenantSuperuser`（`internal/middleware/access.go`）先查集群开关、再查用户属性。

**能力**：

- **进入任意空间**：通过 `X-Tenant-ID` 请求头切换到目标空间，无需在目标空间有 `tenant_members` 行（`IsTenantAccessible` 放行）
- **在目标空间内等同 Admin**：三类守卫（角色守卫 `RequireRole`、归属守卫 `RequireOwnershipOrRole`、handler 侧 `EvaluateOwnershipOrRole`）都在角色不足时兜底放行超管
- **能删除目标空间**：`Owner()` 守卫本质是 `RequireRole(Owner)`，同样有超管兜底，因此超管可删除任意空间（不限于 Owner）
- **调用跨空间端点**：`GET /tenants/all`、`GET /tenants/search`、`POST /tenants`、`GET /tenants` 等跨空间运营端点由 `CrossTenant` 守卫保护，仅超管可访问
- **前端空间选择器**：`canAccessAllTenants=true` 时侧边栏显示空间切换器，可搜索并切换到任意空间

**用途**：多空间运营方、SaaS 平台管理员跨空间运维。

#### 系统管理员（System Admin）

`User.IsSystemAdmin = true`，是**平台级**身份，独立于空间角色矩阵：

- 管理平台级端点：管理其他系统管理员、编辑全局系统设置、跨空间运维面板等（`RequireSystemAdmin` 守卫）
- **不受 `tenant.enable_rbac` 开关影响**：平台级管理端点始终强制鉴权，不会因 RBAC 灰度窗口变成「任意已认证用户可调用」
- 不绑定具体空间角色：系统管理员操作平台级资源时，不经过 Owner/Admin/Contributor/Viewer 阶梯

与跨空间超管的区别：系统管理员管**平台**（全局设置、其他管理员），跨空间超管管**空间内资源**（在目标空间内等同 Admin）。两者正交，可同时授予。

#### API Key 调用

`X-API-Key` 合成的虚拟用户在其所属空间内固定为 Admin（角色守卫对 API Key 短路，走独立的 `APIKeyGate` 能力授权）。脚本集成无需迁移。详见 [API文档概览](../API参考/API文档概览.md)。

#### 孤儿空间自愈

若一个空间在 `tenant_members` 表里没有任何活跃成员（典型场景：仅 API Key 使用过该空间），首位通过认证的真人会被自动晋升为 Owner，避免锁死。
